...
Harness 的“产物”文件芯片点击后调用 `host.openPath`——在**服务器本机**用原生应用打开文件，对远程浏览器毫无意义（无头服务器上还会失败）。经网关访问时，本插件把产物的打开行为全部改为**下载到浏览器**：

1. **客户端**（`lib/client.js`），在带网关标记的页面里：
   - 点击“产物”行内单个文件芯片：直接下载该文件；
   - 点击“产物”块的任意其它位置（“产物”标签、行内空白、`+N 个文件` 折叠计数）：打开“会话产物”面板，通过 `sessions.history` 拉取当前会话全部事件视图，解析出**每一轮的产物路径**完整列出（折叠不再隐藏任何文件），逐行点击即可下载；
   - 补丁 `window.fetch` 拦截 `POST /api/host.openPath`：**消息文本里的行内文件引用（mention）等一切打开入口**都会走下载；下载被网关拒绝（越界/目录等）时回退到原始 RPC 原样转发；
