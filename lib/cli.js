...
//   dsh-web-auth add-user --username <name> [--password <pw> | --password-hash sha256:<hex>]
//   dsh-web-auth remove-user --username <name>
//   dsh-web-auth list
...
function usage() {
  ...
  dsh-web-auth add-user --username <name> [--password <pw> | --password-hash sha256:<hex>] [--work-dir <path>] [--file <path>]
  dsh-web-auth remove-user --username <name> [--file <path>]
...
function genCert(args) {
  ...
  if (args.flags.enable === true) {
    ...
    const existing = rows.find((row) => row !== null && typeof row === "object" && row.id === "web-auth-gateway");
    if (existing !== undefined) {
      existing.config = { ...(existing.config ?? {}), ...config };
    } else {
      rows.push({ id: "web-auth-gateway", config });
    }
...
switch (command) {
  case "init": {
    ...
    if (typeof args.flags.password !== "string") process.stdout.write(`generated password: ${password}\n(store it somewhere safe; regenerate with add-user if lost)\n`);
    break;
  }
  case "add-user": {
    const username = args.flags.username;
    if (typeof username !== "string" || username === "") fail("--username is required");
    const hash = typeof args.flags.passwordHash === "string" ? args.flags.passwordHash : null;
    const password = typeof args.flags.password === "string" ? args.flags.password : null;
    if (hash === null && password === null) fail("provide --password or --password-hash");
    const doc = loadDoc(file);
    const record = { username };
    if (hash !== null) record.passwordHash = hash;
    if (password !== null) record.password = password;
    if (typeof args.flags.workDir === "string" && args.flags.workDir !== "") record.workDir = args.flags.workDir;
    const index = doc.users.findIndex((u) => u?.username === username);
    if (index >= 0) doc.users[index] = record;
    else doc.users.push(record);
    saveDoc(file, doc);
    process.stdout.write(`updated user "${username}" in ${file}\n`);
    break;
  }
  case "remove-user": {
    const username = args.flags.username;
    if (typeof username !== "string" || username === "") fail("--username is required");
    if (username === "admin") fail('the "admin" account cannot be removed');
    const doc = loadDoc(file);
    const before = doc.users.length;
    doc.users = doc.users.filter((u) => u?.username !== username);
    if (doc.users.length === before) fail(`user "${username}" not found in ${file}`);
    saveDoc(file, doc);
    process.stdout.write(`removed user "${username}" from ${file}\n`);
    break;
  }
  case "list": {
    const doc = loadDoc(file);
