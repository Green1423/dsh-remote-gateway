// dsh-login-gateway — the gateway HTTP server: login page + session
  ...
  const begin = (body) => {
    const upstreamReq = httpRequest({
      ...
      upstreamRes.on("end", () => {
        ...
        // The harness serves index.html without cache headers; a browser
        // heuristic cache then pins an OLD client-bundle rev until it
        // expires, so remote users keep running stale plugin code after a
        // refresh. Force revalidation of every proxied HTML document.
        const { "content-length": _dropped, ...headersWithoutLength } = outHeaders;
        res.writeHead(upstreamRes.statusCode ?? 502, upstreamRes.statusMessage, { ...headersWithoutLength, "cache-control": "no-cache" });
        res.end(outBody);
      });
...
// The harness's "产物" (produced files) chips call host.openPath, which opens
// the file in a NATIVE application on the server — meaningless for remote
// browsers (and unavailable on headless hosts). For gateway sessions the
// client half instead intercepts those chips and fetches this endpoint, which
// streams the file with an attachment disposition. Access is limited to the
// account's download root: the account workDir when one is set, else the
// server home directory; realpath is resolved first so symlinks cannot
// escape the root.

/**
 * Resolve and validate one download target against a root directory.
 * @param root - absolute directory the account may download from.
 * @param raw - the raw ?path= value.
 * @returns {ok: true, target, size, name} or {ok: false, status, reason}.
 */
function resolveDownloadTarget(root, raw) {
  if (typeof raw !== "string" || raw === "" || !raw.startsWith("/")) {
    return { ok: false, status: 400, reason: "path must be absolute" };
  }
  let rootReal;
  try {
    rootReal = realpathSync(root);
  } catch {
    return { ok: false, status: 403, reason: "download root unavailable" };
  }
  let target;
  try {
  ...
  if (target !== rootReal && !target.startsWith(rootReal + "/")) {
    return { ok: false, status: 403, reason: "outside the download root" };
  }
  let stat;
  try {
  ...
  if (!stat.isFile()) return { ok: false, status: 403, reason: "not a file" };
  return { ok: true, target, size: stat.size, name: target.slice(target.lastIndexOf("/") + 1) };
}

/** RFC 5987 attachment filename pair (ASCII fallback + UTF-8 star form). */
function attachmentDisposition(name) {
  const ascii = String(name).replace(/[^\x20-\x7e]/g, "_").replace(/["\\]/g, "_") || "download";
  return `attachment; filename="${ascii}"; filename*=UTF-8''${encodeURIComponent(name)}`;
}

/** Serve one authenticated file download (streaming). */
function serveDownload(req, res, session, downloadRootFor) {
  const root = downloadRootFor(session.username);
  if (root === null) return sendJson(res, 403, { error: "downloads are disabled for this account" });
  const resolved = resolveDownloadTarget(root, new URL(req.url ?? "/", "http://x").searchParams.get("path") ?? "");
  if (!resolved.ok) return sendJson(res, resolved.status, { error: resolved.reason });
  res.writeHead(200, {
    "content-type": "application/octet-stream",
    "content-disposition": attachmentDisposition(resolved.name),
  ...
  stream.on("error", (err) => {
    log?.(`web-auth: download stream failed: ${err instanceof Error ? err.message : String(err)}`);
...
export function createGateway(options) {
  const {
    ...
    httpsPort = 8443,
    /**
     * Download root for the produced-file endpoint. Given the authenticated
     * username, returns an absolute root directory (or null to disable
     * downloads). Defaults to the account workDir when {@link getUser}
     * reports one, else the server home directory.
     */
    downloadRootFor = undefined,
  } = options;

  // ── produced-file download roots ───────────────────────────────────────────
  const homeRoot = (() => { try { return homedir(); } catch { return "/"; } })();
  const resolveDownloadRoot = downloadRootFor ?? ((username) => {
    if (typeof getUser === "function") {
      try {
        const user = getUser(username);
        if (user !== undefined && user !== null && typeof user.workDir === "string" && user.workDir.trim() !== "") {
          return user.workDir.trim();
        }
      } catch { /* fall through to the home root */ }
    }
    return homeRoot;
  });
  ...
  const handle = async (req, res, secure) => {
    ...
    // Produced-file download (the remote counterpart of host.openPath).
    if (path === "/dsh-gateway/download") {
      if (req.method !== "GET") {
        res.writeHead(405, { allow: "GET", ...SECURITY_HEADERS });
        return res.end();
      }
      if (logAccess) log(`web-auth: ${session.username} GET /dsh-gateway/download`);
      return serveDownload(req, res, session, resolveDownloadRoot);
    }
