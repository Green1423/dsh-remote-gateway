// dsh-login-gateway — browser half. Renders a floating gear menu whose
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Bump on every produced-panel/client logic change; shown in the panel diagnostics. */
		const PRODUCED_BUILD = "2026-08-26b";
...
.dshAuth_producedFile{display:block;width:100%;box-sizing:border-box;text-align:left;margin:4px 0;padding:6px 10px;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);border-radius:8px;font-size:12px;font-family:ui-monospace,SFMono-Regular,Consolas,"Courier New",monospace;cursor:pointer;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;transition:background-color .12s ease,border-color .12s ease}
.dshAuth_producedFile:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-label-secondary)}
.dshAuth_producedPanel{display:flex;flex-direction:column;max-height:70vh;padding-bottom:12px}
.dshAuth_producedHeader{flex:none}
.dshAuth_producedList{flex:1 1 auto;min-height:0;max-height:none;overflow:auto;margin-top:4px}
.dshAuth_producedPanel .dshAuth_actions{flex:none;margin-top:12px}
`;
		const cssTagId = "dsh-login-gateway/client.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(cssTagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-login-gateway";
		...
		/**
		 * Resolve a produced-file path for the download endpoint: absolute
		 * paths pass through, workspace-relative ones join the session cwd
		 * (mirrors the harness resolveWorkspacePath).
		 * @param cwd - the session workspace root, when known.
		 * @param path - absolute or workspace-relative produced path.
		 * @returns an absolute path when a cwd is available, else the input.
		 */
		function resolveProducedPath(cwd, path) {
			if (typeof path !== "string" || path === "") return path;
			if (path.startsWith("/")) return path;
			if (cwd === undefined || cwd === null || cwd === "") return path;
		...
		function mountProducedFiles(ctx) {
			...
			// 1. Produced-files block clicks. A single file chip downloads that
			//    file directly; clicking ANYWHERE else in the block — the fold
			//    counter ("+N 个文件"), the row background, or the "产物" label —
			//    opens the produced-file panel with the full session list.
			document.addEventListener("click", (event) => {
					...
					if (path !== "") {
						...
						try {
							...
							const snapshot = sessions !== null && sessions !== undefined && sessions.list !== undefined
								...
								: undefined;
							const current = snapshot !== undefined && snapshot !== null ? snapshot.current : undefined;
							const record = typeof current === "string" && current !== "" && snapshot !== undefined && snapshot !== null && snapshot.byId !== undefined
								? snapshot.byId[current]
								: undefined;
							cwd = record !== undefined && record !== null && typeof record.cwd === "string" ? record.cwd : "";
						} catch { /* cwd unavailable — absolute paths still work */ }
						downloadProducedFile(resolveProducedPath(cwd, path));
					}
					return;
				}
				// Row (including the fold counter) or the "产物" label: the
				// label is a sibling span of the row inside the same root, so
				// it is recognized by being a span whose direct parent also
				// directly contains the produced-files row.
				const inRow = target.closest("[data-produced-files-row]") !== null;
				const label = target.closest("span");
				const underLabel = label !== null && label !== undefined
					&& label.parentElement !== null && label.parentElement !== undefined
					&& label.parentElement.querySelector(":scope > [data-produced-files-row]") !== null;
				if (inRow || underLabel) {
					event.preventDefault();
					event.stopPropagation();
					openProducedPanel();
			...
			// The produced-file panel opens INSTANTLY in a loading state and
			// fills in asynchronously: the session.history pull (often tens of
			// thousands of events) happens after the popup is already visible.
			const openProducedPanel = async () => {
				const summary = { sessionId: undefined, events: 0, views: 0, error: "" };
				const panel = renderProducedPanel([], { ...summary, loading: true }, "");
				const paths = [];
				let cwd = "";
				try {
					const sessions = ctx.get("sessions");
					const snapshot = sessions !== null && sessions !== undefined && sessions.list !== undefined
						? sessions.list.getSnapshot()
						: undefined;
					let current = snapshot !== undefined && snapshot !== null ? snapshot.current : undefined;
					if (typeof current !== "string" || current === "") {
						// No active selection — fall back to the first session.
						current = snapshot !== undefined && snapshot !== null && Array.isArray(snapshot.ids) && snapshot.ids.length > 0
							? snapshot.ids[0]
							: undefined;
					}
					summary.sessionId = current;
					if (typeof current === "string" && current !== "" && snapshot !== undefined && snapshot !== null) {
						const record = snapshot.byId !== undefined && snapshot.byId !== null ? snapshot.byId[current] : undefined;
						cwd = record !== undefined && record !== null && typeof record.cwd === "string" ? record.cwd : "";
					}
					...
					if (typeof current === "string" && current !== "" && api !== undefined && api !== null) {
						...
						for (let page = 0; page < 3; page++) {
							...
							if (!result.ok) {
								summary.error = result.error?.message ?? "history failed";
								break;
							}
							...
							for (const entry of events) {
								...
								for (const location of view.locations ?? []) {
									// Locations may be absolute or workspace-relative
									// (e.g. "ppt_assets/fig_1.html"); both are kept and
									// resolved against the session cwd at download time.
									const produced = location !== null && typeof location === "object" ? location.path : undefined;
									if (typeof produced === "string" && produced !== "" && !paths.includes(produced)) paths.push(produced);
								}
							}
						...
						summary.error = "no session available";
					}
				} catch (err) {
					summary.error = err instanceof Error ? err.message : String(err);
				}
				// History yielded nothing — at least list the visible chips.
				if (paths.length === 0) {
					try {
						for (const chip of document.querySelectorAll("[data-produced-files-row] button[title]")) {
							const produced = chip.getAttribute("title");
							if (typeof produced === "string" && produced !== "" && !paths.includes(produced)) paths.push(produced);
						}
					} catch { /* DOM fallback is best effort */ }
				}
				panel.update(paths, summary, cwd);
			};
				...
				window.fetch = async (input, init) => {
					...
					if (method === "POST" && (pathOnly === "/api/host.openPath" || pathOnly.endsWith("/api/host.openPath"))) {
						...
						if (typeof target === "string" && target.startsWith("/")) {
							// Remote openPath is meaningless (native open on the
							// server) — download the file, and answer the caller
							// with a synthesized success EITHER WAY so the UI
							// never shows the server's xdg-open failure popup.
							// A refused download is logged and stays silent.
							await downloadProducedFile(target, originalFetch);
		...
		/**
		 * Render the produced-file panel. Opens instantly (optionally in a
		 * loading state) and returns an update handle the async loader calls
		 * once the produced paths are known — the popup never waits for the
		 * session.history pull.
		 * @param paths - initial produced paths (usually [] while loading).
		 * @param summary - diagnostic counters ({sessionId, events, views, error, loading}).
		 * @param cwd - session workspace root for relative-path resolution.
		 * @returns {{update(paths, summary, cwd): void, root: HTMLElement}}
		 */
		function renderProducedPanel(paths, summary = {}, cwd = "") {
			// Re-opening refreshes the panel instead of silently keeping a stale
			// (possibly empty) instance.
			const existing = document.getElementById("dsh-auth-produced-root");
			if (existing !== null) existing.remove();
			const root = document.createElement("div");
			root.id = "dsh-auth-produced-root";
			const descEl = h("div", { class: "dshAuth_desc" });
			const listEl = h("div", { class: "dshAuth_producedList" });
			// Title/desc stay pinned at the top and the close action at the
			// bottom; only the produced list scrolls in between.
			const panel = h("div", { class: "dshAuth_panel dshAuth_producedPanel", style: "bottom:110px" },
				h("div", { class: "dshAuth_producedHeader" },
					h("div", { class: "dshAuth_title" }, "会话产物"),
					descEl,
				),
				listEl,
				h("div", { class: "dshAuth_actions" },
					h("button", { class: "dshAuth_btn dshAuth_btnGhost", type: "button", onClick: () => { root.remove(); cleanup(); } }, "关闭"),
				),
			);
			root.appendChild(panel);
			document.body.appendChild(root);
			// Clicking anywhere OUTSIDE the panel closes it; Escape closes it
			// too. The click that OPENED the panel is stopped in the capture
			// phase, so it never reaches these bubble-phase listeners.
			const cleanup = () => {
			...
			const onDocClick = (event) => {
				const target = event.target;
				if (target !== null && target !== undefined && root.contains(target)) return;
				root.remove();
				cleanup();
			};
			...
			const update = (nextPaths, nextSummary = {}, nextCwd = "") => {
				const loading = nextSummary.loading === true;
				const detail = loading
					? "正在加载产物列表…"
					: nextPaths.length > 0
						? "点击文件即可下载（共 " + String(nextPaths.length) + " 个）"
						: nextSummary.error !== undefined && nextSummary.error !== null && nextSummary.error !== ""
							? "未找到产物文件（" + nextSummary.error + "）"
							: "未找到产物文件"
								+ (nextSummary.sessionId !== undefined
									? "（会话 " + String(nextSummary.sessionId).slice(0, 24) + "，事件 " + String(nextSummary.events ?? 0) + "，视图 " + String(nextSummary.views ?? 0) + "，构建 " + PRODUCED_BUILD + "）"
									: "（无会话）");
				descEl.textContent = detail;
				// Absolute paths download directly; workspace-relative ones
				// resolve against the session cwd (mirrors resolveWorkspacePath).
				const resolvePath = (path) => resolveProducedPath(nextCwd, path);
				listEl.textContent = "";
				for (const path of nextPaths) {
					listEl.appendChild(h("button", {
						...
						type: "button",
						title: path,
						onClick: () => downloadProducedFile(resolvePath(path)),
					}, path.split(/[\\/]/).pop() ?? path));
				}
			};
			update(paths, summary, cwd);
			return { update, root };
		}

		function apply(ctx) {
			const controller = new GatewayController();
			ctx.effect(() => () => controller.dispose(), "dsh-login-gateway: controller");
