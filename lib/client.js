...
`;
		...
		function mountProducedFiles(ctx) {
			...
			// The produced-file panel: pulls every produced path of the
			// current session from session.history event views. Falls back to
			// the visible chips when history yields nothing, and reports a
			// diagnostic line in the panel so an empty result can be traced.
			const openProducedPanel = async () => {
				...
				renderProducedPanel(paths, summary, cwd);
			};
		...
		function renderProducedPanel(paths, summary = {}, cwd = "") {
			...
			const detail = paths.length > 0
				? "点击文件即可下载（共 " + String(paths.length) + " 个）"
				: summary.error !== undefined && summary.error !== null && summary.error !== ""
					? "未找到产物文件（" + summary.error + "）"
					: "未找到产物文件"
						+ (summary.sessionId !== undefined
							? "（会话 " + String(summary.sessionId).slice(0, 24) + "，事件 " + String(summary.events ?? 0) + "，视图 " + String(summary.views ?? 0) + "）"
							: "（无会话）");
			const panel = h("div", { class: "dshAuth_panel", style: "bottom:110px" },
				h("div", { class: "dshAuth_title" }, "会话产物"),
				h("div", { class: "dshAuth_desc" }, detail),
				...paths.map((path) => h("button", {
					class: "dshAuth_producedFile",
					type: "button",
					title: path,
					onClick: () => downloadProducedFile(resolveProducedPath(cwd, path)),
				}, path.split(/[\\/]/).pop() ?? path)),
				h("div", { class: "dshAuth_actions" },
