...
`;
		...
		/**
		 * Remote produced-file download: the harness "产物" chips call
		 * host.openPath, which opens the file natively ON THE SERVER —
		 * meaningless for a remote browser (and unavailable on headless
		 * hosts). When the page was reached through the gateway, intercept
		 * those chip clicks (capture phase, before the conversation's own
		 * handler) and download the file through the gateway's
		 * /dsh-gateway/download endpoint instead. The chips carry the
		 * absolute path in their title attribute; the gateway validates it
		 * against the account's download root (workDir or home).
		 */
		function mountProducedDownload() {
			if (typeof document === "undefined") return;
			const download = async (path) => {
				try {
					const response = await fetch("/dsh-gateway/download?path=" + encodeURIComponent(path));
